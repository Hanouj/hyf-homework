...
function filterYearListings(movies, filterObjectYears) {
  ...
  return movies.filter(
    (oneYear) => oneYear.year <= maxYear && oneYear.year >= minYear
  );
}
...
console.log(
  `Between the years 1980-1989 there are ${filteredYears.length} movies made.`
); // output Between the years 1980-1989 there are 6 movies made.

//4-extra key called tag

const newMovieArrayWithNewTag = movies.map((newKey) => {
  return newKey.rating >= 7
    ? { ...newKey, tag: "Good" }
    : newKey.rating >= 4 && newKey.rating < 7
    ? { ...newKey, tag: "Average" }
    : newKey.rating < 4
    ? { ...newKey, tag: "Bad" }
    : newKey;
...
const filteredMovieWords = movies.filter((oneMovie) =>
...
const totalMoviesRating = movies.reduce((total, firstRating) => {
...
const totalNumberForEachTag = newMovieArrayWithNewTag.reduce(
  (total, oneMovie) => {
